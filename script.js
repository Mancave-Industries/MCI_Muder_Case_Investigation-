...
function informationBase() {
  ...
  app.innerHTML = `<section class="screen" ${bg(ASSETS.casefile)}><div class="content"><div class="panel dark"><h2>INFORMATION BASE</h2><p>A Blackwood reference file for lore, deduction and case review.</p><div class="info-tabs">${tabs.map(([id, label]) => `<button onclick="setInfoTab('${id}')">${label}</button>`).join("")}</div></div><div class="info-grid">${items.map(renderInfoEntry).join("")}</div><button class="primary" onclick="go('home')">RETURN HOME</button></div></section>`;
}
...
function renderInfoEntry(entry) {
  ...
  return `<div class="info-entry"><img class="${contain}" src="${img}" alt="${escapeHTML(name)}" onerror="this.style.opacity=.15"><div><h3>${escapeHTML(name)}</h3><p class="small">${label}</p><div class="playable-attributes">${details.map(d => `<span class="playable-attribute">${escapeHTML(d)}</span>`).join("")}</div><p class="small">Blackwood file entry. Further background will unlock as the season progresses.</p></div></div>`;
}
...
function dataAudit() {
  const rows = [
    ...
    ["Information Event", "Knowledge transfer", "Result screen", "After case", "If present"],
...
function investigation() {
  ...
  app.innerHTML = `<section class="screen" ${bg(ASSETS.investigation)}><div class="topbar"><div class="hudrow"><div class="hudcell"><span class="hudlabel">Detective:</span><span class="hudvalue">${escapeHTML(player.name || player.rank)}</span></div><div class="hudcell"><span class="hudlabel">Case:</span><span class="hudvalue">${state.caseIndex + 1}</span></div><div class="hudcell"><span class="hudlabel">Accusations Remaining:</span><span class="hudvalue">${state.guessesLeft}</span></div></div></div><div class="content">${progressHTML()}${accusationGrid()}${evidenceBoard()}${carousel("suspect", "SUSPECT", visibleCards("suspect"))}${carousel("weapon", "WEAPON", visibleCards("weapon"))}${carousel("room", "ROOM", visibleCards("room"))}${carousel("motive", "MOTIVE", visibleCards("motive"))}<div class="panel"><h2>MAKE ACCUSATION</h2><p>Select one suspect, weapon, room and motive.</p><button class="primary" onclick="accuse()">SUBMIT ACCUSATION</button></div>${bottomHUD()}</div></section>${accusationPending ? loaderOverlay() : ""}`;
...
function card(type, itemData) {
  ...
  return `<div class="card ${cls}" data-card-id="${id}" onclick="selectCard('${type}','${id}')">
    <img class="${contain}" src="${img}" alt="${escapeHTML(displayName)}" onerror="this.style.opacity=.15">
    <div class="card-role">${CARD_LABELS[type]}</div>
...
function bottomHUD() {
  ...
  return `<div class="bottomhud"><h2>LAST ACCUSATION FEEDBACK</h2><div class="feedback-grid">${state.feedback.map(f => {
    if (f.ok) {
      return `<div class="feedback-line"><b>${labelFor(f.type)}</b><br><span class="green">${labelFor(f.type)} confirmed:</span> ${escapeHTML(f.name)}</div>`;
    }
...
function narrativeOutcomeHTML(c, failed) {
  const sections = [
    ...
    ["INFORMATION EVENT", c.narrative.informationEvent],
